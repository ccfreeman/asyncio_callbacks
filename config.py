...
_config_file  = os.path.join(_dir_path, 'asyncio_callbacks.config')
...
class Config(object):
    ...
    FNAME = 'asyncio_callbacks'+ datetime.datetime.now().strftime('%Y-%m-%d')+'.log'
    LOGGING_CONFIG['handlers']['file_handler']['filename'] = os.path.join(LOG_PATH, FNAME)

    logging.config.dictConfig(LOGGING_CONFIG)
    LOGGER = logging.getLogger(__name__)

    NUM_FORECASTS = os.environ.get("NUM_FORECASTS")
    if NUM_FORECASTS is None:
        NUM_FORECASTS = node.xpath('//add[@key="numForecasts"]')[0].attrib['value']
